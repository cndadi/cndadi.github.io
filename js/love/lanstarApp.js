...
let lanstar = {
    ...
    addArticleLike: () => {
        ...
        $('.artices').on('click','.post-like', function () {
            ...
            $.ajax({
                ...
                //  璇锋眰鎴愬姛鐨勫嚱鏁 
                success: (data) => {
                    let re = /\d/;  //  鍖归厤鏁板瓧鐨勬鍒欒〃杈惧紡
                    //  鍖归厤鏁板瓧
                    if (re.test(data)) {
                        //  鎶婄偣璧炴寜閽腑鐨勭偣璧炴暟閲忚缃负浼犲洖鐨勭偣璧炴暟閲 
    ...
    addPageLike: ()=>{
        ...
        $('#agree-btn').on('click', function () {
            ...
            $.ajax({
                //  璇锋眰鏂瑰紡 post
                type: 'post',
                //  url 鑾峰彇鐐硅禐鎸夐挳鐨勮嚜瀹氫箟 url 灞炴€ 
                //  鍙戦€佺殑鏁版嵁 cid锛岀洿鎺ヨ幏鍙栫偣璧炴寜閽殑 cid 灞炴€ 
                data: 'agree=' + $(this).attr('data-cid'),
                async: true,
                timeout: 30000,
                cache: false,
        ...
        $('.protected-btn').click(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.indexOf("瀵嗙爜閿欒") >= 0 && data.indexOf("<title>Error</title>") >= 0) {
                        return Toastify({
                            text: '瀵嗙爜閿欒锛岃閲嶈瘯锛 ',
                            duration: 3000,
                            backgroundColor: "#c02c38"
    ...
    addCommentSecret: () => {
        ...
        $('#secret-button').click(function () {
            ...
            if ($(this).is(':checked')) {
                textareaDom.attr('placeholder', '绉佸瘑鍥炲涓 ')
            } else {
    ...
    addMorePages(){
        $('.next').click(function() {
            $this = $(this);
            $(this).addClass('loading').text('姝ｅ湪鍔姏鍔犺浇');
            ...
            if (href != undefined) {
                $.ajax({
                    ...
                    success: function(data) { //璇锋眰鎴愬姛
                        $this.removeClass('loading').text('鏌ョ湅鏇村');
